...
def main(ref_file, pred_file, iterations=5,
         lc_file=None, slope_file=None, aspect_file=None, get_ndvi=False):
    out_data = {}
    ...
    if get_ndvi:
        for NDVImin, NDVImax in zip([-1.0, 0, 0.5, 0.75], [0, 0.5, 0.75, 1]):
            ...
            out_data[f'ndvi_{NDVImin}_{NDVImax}'] = [accuracy*100, precision*100, recall*100, F1*100]
    ...
    if slope_file:
        for slopemin, slopemax in zip([0, 5, 10, 20, 30], [5, 10, 20, 30, 90]):
            ...
            out_data[f'slope_{slopemin}_{slopemax}'] = [accuracy*100, precision*100, recall*100, F1*100]
    ...
    if aspect_file:
        for aspectmin, aspectmax in zip([0, 90, 180, 270], [90, 180, 270, 360]):
            ...
            out_data[f'aspect_{aspectmin-45}_{aspectmax-45}'] = [accuracy*100, precision*100, recall*100, F1*100]
    ...
    if lc_file:
        for lc in np.unique(lc_data):
            ...
            if TP > 0:
                ...
                out_data[f'lc_{lc}'] = [accuracy * 100, precision * 100, recall * 100, F1 * 100]
            else:
                print("\tNo TP (N/A)")

                out_data[f'lc_{lc}'] = [-1, -1, -1, -1]
